...
def bert_cos_score_idf_all(
    ...
    device = next(model.parameters()).device
    batch_size = 1
    iter_range1 = range(0, len(refs), batch_size)
    iter_range2 = range(0, len(hyps), batch_size)
    if verbose:
        print("computing greedy matching all.")
        iter_range1 = tqdm(iter_range1)
        iter_range2 = tqdm(iter_range2)
    ...
    with torch.no_grad():
        i = 0
        for batch_start1 in iter_range1:
            for batch_start2 in iter_range2:
                batch_refs += refs[batch_start1 : batch_start1 + batch_size]
                batch_hyps += hyps[batch_start2 : batch_start2 + batch_size]
                if i%1024 == 1023:
                    ref_stats = pad_batch_stats(batch_refs, stats_dict, device)
                    hyp_stats = pad_batch_stats(batch_hyps, stats_dict, device)

                    P, R, F1 = greedy_cos_idf(*ref_stats, *hyp_stats, all_layers)
                    # preds.append(torch.stack((P.detach(), R.detach(), F1.detach()), dim=-1).cpu())
                    preds.append((P.detach(), R.detach(), F1.detach()))
                    # del ref_stats
                    # del hyp_stats
